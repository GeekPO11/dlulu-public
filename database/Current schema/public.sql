...
CREATE TABLE public.plan_entitlements (
  ...
  max_active_goals integer NOT NULL,
  token_hard_cap bigint,
  token_soft_cap bigint,
  calendar_sync_enabled boolean DEFAULT false,
  warning_thresholds jsonb DEFAULT '{}'::jsonb,
...
CREATE TABLE public.user_entitlement_overrides (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL UNIQUE,
  override_plan_id text NOT NULL,
  reason text,
  starts_at timestamp with time zone DEFAULT now(),
  ends_at timestamp with time zone,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT user_entitlement_overrides_pkey PRIMARY KEY (id),
  CONSTRAINT user_entitlement_overrides_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id),
  CONSTRAINT user_entitlement_overrides_override_plan_id_fkey FOREIGN KEY (override_plan_id) REFERENCES public.plan_entitlements(plan_id)
);
CREATE TABLE public.user_usage_periods (
  ...
  tokens_total_used bigint DEFAULT 0,
  tokens_input_used bigint DEFAULT 0,
  tokens_output_used bigint DEFAULT 0,
  active_goal_count integer DEFAULT 0,
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT user_usage_periods_pkey PRIMARY KEY (id),
  CONSTRAINT user_usage_periods_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);